...
async def main(host, port, buffer_size, sample_rate, image_path):
    ...
    client_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Disable Nagle's algorithm for real-time data
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='TCP client to read and print data in real-time.')
    parser.add_argument('--host', type=str, default='localhost', help='Host IP address to connect to')
    parser.add_argument('--port', type=int, default=9002, help='TCP port number to connect to')
